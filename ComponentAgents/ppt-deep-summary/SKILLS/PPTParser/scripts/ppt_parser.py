...
def extract_chart(shape) -> ChartData | None:
    ...
    try:
        ...
        try:
            ...
            if plot:
                series_count = len(list(plot.series)) if hasattr(plot, 'series') else 0
                try:
                    category_count = len(plot.categories) if hasattr(plot, 'categories') else 0
                except Exception:
                    category_count = 0
            has_legend = bool(chart.has_legend) if hasattr(chart, 'has_legend') else False
        except Exception:
            pass
...
def generate_markdown(s: SlideData) -> str:
    ...
    if s.images:
        lines.extend(["### Images"] + [
            f"- {i.filename} ({i.width:.1f}x{i.height:.1f} in)" +
            (f" [{i.shape_name}]" if i.shape_name else "") +
            (f" @({i.left:.1f},{i.top:.1f})" if i.left or i.top else "")
            for i in s.images] + [""])
    if s.hyperlinks:
    ...
    if s.charts:
        lines.extend(["### Charts"])
        for c in s.charts:
            chart_desc = f"- Type: {c.chart_type}"
            if c.title:
                chart_desc += f"\n  Title: {c.title}"
            if c.series_count:
                chart_desc += f"\n  Series: {c.series_count}, Categories: {c.category_count}"
            if c.has_legend:
                chart_desc += "\n  Has legend"
