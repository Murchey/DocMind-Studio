...
## 🔒 Step 9b-int: 三线表确认（强制交互检查点）

**此步骤必须向用户询问三线表格式化选项，使用 AskUserQuestion 工具：**

在 Step 9a 格式标准化完成后、Step 9b 表格格式化之前，检测文档中是否存在表格。如果检测到表格，**必须**向用户确认：

1. **是否启用三线表格式**：显示检测到的表格数量，询问用户是否应用三线表格式化
2. **三线表样式选择**（如用户选择启用）：展示线宽选项

示例问题格式：
```
问题1: 文档中检测到 {N} 个表格，是否应用三线表格式？
选项: [是-全部应用三线表(推荐)] [是-仅数据表不处理题注] [否-跳过表格格式化]
```
...
**执行后**：将用户选择写入 `workspace/validated/table_config.json`：
```json
{
  "apply_three_line": true,
  "mode": "all",
  "confirmed_at": "2026-06-09T12:00:00"
}
```

## Step 9b: 表格格式化处理
根据 `Step 9b-int` 的用户确认结果，调用 `table-processor` 对文档中的表格进行格式化。

**此步骤根据用户选择执行，不再自动全部处理。**
